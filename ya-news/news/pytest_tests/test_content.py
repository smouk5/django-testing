...
class TestContent:
    ...
    def test_news_sorted_from_new_to_old(self, client):
        # Создаём две новости и пытаемся (если возможно) выставить разное время
        now = timezone.now()
        older = News.objects.create(title='Old', text='Text')
        newer = News.objects.create(title='New', text='Text')

        # Попробуем принудительно выставить дату, если поле есть
        ...
        # ВАЖНО: response.context['object_list'] — QuerySet, у него нет [-1]
    ...
    def test_comments_sorted_old_to_new_on_detail(self, client, author, news):
        ...
        # Принудительно выставим время, если поле есть
        ...
        # В разных версиях проекта комментарии могут лежать по-разному:
        if 'comments' in response.context:
            comments = list(response.context['comments'])
        elif 'news' in response.context:
            # fallback: если в контексте лежит объект новости
            comments = list(response.context['news'].comment_set.all())
        else:
            # На всякий случай — чтобы ошибка была понятной
            raise AssertionError('Не найден список комментариев в контексте detail-страницы.')
