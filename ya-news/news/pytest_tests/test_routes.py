...
class TestRoutes:
    ...
    def test_auth_pages_available_for_anonymous(self, client):
        # login + signup обычно OK по GET
        for name in ('users:login', 'users:signup'):
            url = reverse(name)
            response = client.get(url)
            assert response.status_code == HTTPStatus.OK

        # logout в Django часто POST или GET->редирект. Главное: маршрут существует.
    ...
    def test_authenticated_user_cannot_edit_or_delete_other_users_comment(self, reader_client, comment):
        for name in ('news:edit', 'news:delete'):
            url = reverse(name, args=(comment.id,))
